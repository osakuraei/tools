...
{
  chomp($primer_id);
  my $sth = $dbh->prepare("select sn,primer_id,plasmid_place,plate_well,stock_place,vector from _cs_transfer_result where (vector like '%M13%')and primer_id=\'$primer_id\' limit 1");
  $sth->execute();
  my $ref;
  my $numRows = $sth->rows;
   if($numRows>0)
