...
{
  chomp($primer_id);
  my $sth = $dbh->prepare("select sn,primer_id,plate_well,stock_place,vector from _cs_transfer_result where (vector like '%lx304%' or vector like '%Lv122%')  and primer_id=\'$primer_id\'");
  $sth->execute();
  my $ref;
  my $numRows = $sth->rows;
  if($numRows>0)
  {
	  while ($ref = $sth->fetchrow_hashref())
	    {
	        print RESULT $ref->{'primer_id'},"\t",$ref->{'sn'},"\t",$ref->{'plate_well'},"\t",$ref->{'stock_place'},"\t",$ref->{'vector'},"\n";
	    }
  }
  else
    {
